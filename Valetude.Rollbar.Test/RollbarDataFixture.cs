...
namespace Rollbar.Test {
    public class RollbarDataFixture {
        private static readonly Regex WhiteSpace = new Regex(@"\s");
        private readonly RollbarData _basicData;

        public RollbarDataFixture() {
            this._basicData = new RollbarData("environment", new RollbarBody("test")) {Timestamp = null, Platform = null, Language = null};
        }
        ...
        public void Notifier_is_setup() {
            ...
            Assert.Equal(_basicData.Notifier["name"], "Valetude.Rollbar");
            var version = typeof (RollbarData).Assembly.GetName().Version.ToString(3);
            Assert.Equal(string.Format("{{`name`:`Valetude.Rollbar`,`version`:`{0}`}}", version), WhiteSpace.Replace(JsonConvert.SerializeObject(_basicData.Notifier), "").Replace('"', '`'));
        }
        ...
        public void Environment_is_mandatory() {
            Assert.Throws<ArgumentNullException>(() => {
                new RollbarData(null, A.Fake<RollbarBody>());
            });
        }
        ...
        public void Platform_default_can_be_overriden() {
            ...
            try {
                RollbarData.DefaultPlatform = "mono";
                var data = new RollbarData("whatever", A.Fake<RollbarBody>());
                Assert.Equal("mono", data.Platform);
            }
            finally {
                RollbarData.DefaultPlatform = defaultPlatform;
            }
        }
        ...
        public void Timestamp_shows_up_in_json() {
            var rollbarData = new RollbarData("env", new RollbarBody("test"));
        ...
        public void Person_shows_up_in_json() {
            var rp = new RollbarPerson("person-id");
        ...
        public void GuidUpdatesUuid() {
            var rd = new RollbarData("whatever", A.Fake<RollbarBody>());
            Assert.Null(rd.Uuid);
        ...
        private void TestOptionalDataProperty<T>(string jsonName, Action<RollbarData, T> set, T value) where T : class {
